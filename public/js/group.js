...
function getApi() {
    ...
    axios.get("http://127.0.0.1:8000/api/group", { params }).then((res) => {
        data = res;
        console.log(data);
...
$(".submit").click(function (e) {
    ...
    if (e.target.id === 'update-submit') {
        ...
        axios.put('http://127.0.0.1:8000/api/group/' + updateId, JSON.stringify(updateData))
...
$("#delete").click(function (e) {
    ...
    // 刪除
    axios.delete(`http://127.0.0.1:8000/api/group/${checkedList}`)
        .then(function (response) {
...
document.addEventListener('apiDataReady', function () {
    ...
    data["data"]["data"].forEach((element) => {
        ...
        $("tbody").append(`<td>${element["descript"]}</td>`);
    ...
    $(".fa-pen").click(function (e) {
        ...
        // 取得單筆
        axios.get(`http://127.0.0.1:8000/api/group/${updateId}`)
            .then(function (response) {
                console.log('成功', response.data);
                data = response.data
                let newBackgroundImage = `url("${data.logo}")`;
                $("#name").val(data.name);
